...
* integrates the latest version of Extra Icons (2026.1.4):
  ...
  * rework TypeScript support: let the IDE show the default TypeScript icons for `*.ts` files when using the New UI, and show the New UI's TypeScript icons when using the Classic UI. Other TypeScript icons (for `*.spec.ts`, etc.) are not affected. This should offer a better experience when working with TypeScript files.
