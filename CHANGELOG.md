...
* integrates the latest version of Extra ToolWindow Colorful Icons (2025.1.1):
  ...
  * rework the `Back`, `Bookmarks`, `Branch`, `Collapse`, `Collapse All`, `Changes` (VCS), `Commit`, `Debug`, `Expand`, `Expand All`, `Forward`, `Gradle`, `Learn` (Feature Trainer), `Maven`, `Merge`, `Redo`, `Run`, `Problems`, `Push`, `Rollback`, `Services`, `Show Diff`, `Terminal`, `Undo`, `Update`, and `Writerside` tool window icons when using the New UI.
